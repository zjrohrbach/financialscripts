...
acct_options=( bank1 bank2 bank3 bank4 )	#list of possible accounts
path_to_finances=/Users/zacharyrohrbach/Desktop/git/testdata/ #path to saving location [Make sure to include closing /]
...
then
	export L_DL_ACCT=${acct_options[0]}
fi
...
then
	...
	let n=0
	for i in ${acct_options[@]} #list all posible accounts
	do 
		printf "%s\t" "$n=$i"
		let n++
	done
	echo ""
	read -p ">  " promptvar
	export L_DL_ACCT=${acct_options[$promptvar]}
...
then
	...
	printf "%15s %15s %15s" "" "csv" "pdf" #heading
	printf "\n" ""
	for i in ${acct_options[@]}  #foreach account, check both for the csv and the pdf
	do
		printf "%15s" "$i"
		for j in 'c' 's'
		do
			...
			checkfilename="$j-$i-$L_DL_MONTH.$ext"
